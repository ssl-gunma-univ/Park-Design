<template>
  <div class="row m-0" :style="{'background-image': 'url(' + require('@/assets/Images/green.jpg') + ')', 'height': '100vh', 'overflow': 'auto'}">
    <main class="container-fluid offset-2 col-7 bg-white border shadow-lg" style="height: 100vh;">
      <div class="border text-center" style="height: 50vh;">
        <img v-if="room.indexnum === -1" class="w-50 img-fluid my-3 mx-auto d-block" alt="image" src="@/assets/AnyamonyaCards/card.jpg">
        <img v-else class="w-50 img-fluid my-3 mx-auto d-block" alt="image" :src='require("@/assets/AnyamonyaCards/" + cardType[room.indexnum] + ".jpg")'>
      ...
      <div class="my-3">
        ...
        <button @click="fornextimage" :disabled="! room.isNextEnabled || nextclicked || ! playing || getUsername(room.currentTurnIdx) !== me.username">めくる</button>
      </div>

      <div v-if="room.isNextEnabled || (deck[room.indexnum] !== undefined && deck[room.indexnum].name !== '') || getUsername(room.currentTurnIdx) !== me.username">
        <input
      ...
      <div v-else>
        <input
          ...
          placeholder="Type a name..."
        >
        <button type="button" @click="saveName" :disabled="room.isNextEnabled || (deck[room.indexnum] !== undefined && deck[room.indexnum].name !== '')">命名</button>
      </div>
...
export default {
  ...
  methods: {
    ...
    nextimage() {
      ...
      this.image = require("@/assets/AnyamonyaCards/" + this.cardType[this.room.indexnum] + ".jpg");
      this.name = "";
    ...
    typeName () {
      if (this.name.length > 0) {
        ...
        if(this.name === this.deck[this.room.indexnum].name && ! this.deck[this.room.indexnum].isCalled) {
